// ==========================================
// 1. ИНИЦИАЛИЗАЦИЯ, ТЕМЫ И КАСТОМНЫЙ ALERT
// ==========================================
...
// Кастомная замена системного alert()
        ...
        tab.innerHTML = `
            ...
            ${forms.length > 1 ? `
                <span class="tab-actions">
                    <button class="tab-action-btn" onclick="deleteForm('${form.id}', event)">
...
// ==========================================
// 3. ДВИЖОК ТЕСТИРОВАНИЯ И РЕНДЕР ВОПРОСОВ
// ==========================================
...
function renderQuestion() {
    ...
    if (q.type === 'radio') {
        ...
        html += `<select id="q_select" class="admin-input">
...
// ==========================================
// 4. ДРАГ-ЭНД-ДРОП, ОТВЕТЫ И ИТОГИ
// ==========================================
...
function finishQuiz() {
    ...
    form.questions.forEach((q, idx) => {
        ...
        reviewHtml += `
            <div class="review-item ${isCorrect ? 'correct-item' : 'incorrect-item'}">
                <strong>${idx + 1}. ${q.title}</strong><br/>
                <span class="${isCorrect ? 'text-success' : ''}">
...
// ==========================================
// 5. АДМИНКА, ИМПОРТ/ЭКСПОРТ И ИНИЦИАЛИЗАЦИЯ
// ==========================================
function tryLogin() {
...
function addQuestion() {
    ...
    alert('Вопрос успешно добавлен!', 'check_circle');
}
    ...
    form.questions.forEach((q, idx) => {
        ...
        card.innerHTML = `
            ...
            <div class="gcard-actions">
                <button class="btn-delete" onclick="deleteQuestion(${idx})">
    ...
    reader.onload = function(e) {
        try {
            ...
            if (imported && imported.title && Array.isArray(imported.questions)) {
                ...
                alert('Форма успешно импортирована!', 'check_circle');
            } else {
